...
# ================================================================================
# VOLUME VIEWMODEL CLASS UNIT TEST SUMMARY
...
# 3. Viewmodel notifies only when model changes. (Done)
# ================================================================================


class TestVolumeViewModel:
    # Test: Volume viewmodel correctly exposes volume model properties.
    def test_viewmodel_correctly_exposes_model_properties(self, volume_model, volume_viewmodel):
        # --- Assert ---
        assert volume_viewmodel.current_volume == volume_model.current_volume
        assert volume_viewmodel.is_muted == volume_model.is_muted
    ...
    # Test case: Viewmodel notifies only when model changes.
    @pytest.mark.parametrize("method_name, value", [
        ("set_volume", 100),
        ("set_mute", False)
    ], ids=["set_volume", "set_mute"])
    def test_viewmodel_notifies_when_model_changes(
            self,
            volume_model,
            volume_viewmodel,
            method_name,
