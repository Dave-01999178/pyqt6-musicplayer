...
def extract_id3_tags(
    ...
    """Extract ID3 metadata tags from an MP3 audio file.
    ...
    Args:
        ...
        defaults: A dataclass that contains fallback values for the missing metadata
                  tags.

    Returns:
        AudioInfoDict: A dictionary that contains metadata tags (title, artist, album,
                       and duration) as keys and their corresponding values as values.
...
def get_metadata(audio: FileType) -> AudioInfoDict:
    ...
    Args:
        audio: An audio object returned by `mutagen.File` (e.g. MP3, FLAC, OggVorbis,
               or WAVE).
